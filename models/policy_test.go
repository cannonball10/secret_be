...
func TestNewEnactedPolicy(t *testing.T) {
	p := NewEnactedPolicy("G1", 2, secrethitler.PolicyLiberal, "GOV1", false)
	if p.GameID != "G1" || p.Sequence != 2 || p.Type != secrethitler.PolicyLiberal {
		t.Errorf("fields = %+v", p)
	}
...
func TestEnactedPolicyKeys(t *testing.T) {
	p := NewEnactedPolicy("G1", 7, secrethitler.PolicyFascist, "", true)
	if p.PK() != "GAME#G1" {
		t.Errorf("PK() = %q", p.PK())
	}
	if p.SK() != "POLICY#0007" {
...
func TestPolicyOrdering(t *testing.T) {
	...
	p1 := NewEnactedPolicy("G1", 2, secrethitler.PolicyLiberal, "", false).SK()
	p2 := NewEnactedPolicy("G1", 10, secrethitler.PolicyLiberal, "", false).SK()
	if !(p1 < p2) {
